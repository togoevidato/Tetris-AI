    ...
    var KEY     = { ESC: 27, SPACE: 32, LEFT: 37, UP: 38, RIGHT: 39, DOWN: 40, H: 72, B: 66 },
    ...
    function keydown(ev) {
        ...
        if (playing) {
            switch(ev.keyCode) {
                ...
                case KEY.H:
                    ai_mode = "heuristic";
                    console.log("AI Mode = heuristic");
                    handled = true;
                    break;
                case KEY.B:
                    ai_mode = "beam";
                    console.log("AI Mode = beam search");
                    handled = true;
                    break;
                case KEY.ESC:
                    lose();
    ...
    function removeLines() {
        ...
        //FIX Bug 6: Starting from ny-1 instead on ny.
        for(y = ny-1 ; y >= 0 ; --y) {
            complete = true;
            for(x = 0 ; x < nx ; ++x) {
                if (!getBlock(x, y)) {
    ...
    function removeLine(n) {
        ...
        for(y = n ; y >= 0 ; --y) {
            for(x = 0 ; x < nx ; ++x)
                //FIX Bug 7: Getting 0 values instead of null. 
